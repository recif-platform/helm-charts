...
```bash
curl -sSL https://raw.githubusercontent.com/recif-platform/recif/main/deploy/install.sh | bash
```
...
|---------|-----|---------|-------------|
| **api** | `api.image` | `ghcr.io/recif-platform/recif-api` | API server image |
...
| **operator** | `operator.image` | `ghcr.io/recif-platform/recif-operator` | Operator image |
| | `operator.replicas` | `1` | Replica count |
| **dashboard** | `dashboard.enabled` | `true` | Deploy the web dashboard |
| | `dashboard.image` | `ghcr.io/recif-platform/recif-dashboard` | Dashboard image |
...
| **corail** | `corail.image` | `ghcr.io/recif-platform/corail` | Default agent runtime image |
